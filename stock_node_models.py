...
class OriginalMambaBGNNPerStock(nn.Module):
    ...
    def __init__(self, config: StockGraphModelConfig, mc_train: int = 1, mc_eval: int = 3):
        super().__init__()
        args = ModelArgs(d_model=config.num_features, seq_len=config.lookback, d_state=64)
        self.num_stocks = config.num_stocks
        self.model = MAMBA_BayesMAGAC(args, R=2, K=2, d_e=8, heads=2, mc_train=mc_train, mc_eval=mc_eval)
...
class StockNodeMambaBGNN(nn.Module):
    ...
    def _adjacency(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        hybrid = 0.5 * self.static_adjacency.to(adaptive_adj.device) + 0.5 * adaptive_adj
        hybrid = hybrid / hybrid.sum(dim=1, keepdim=True).clamp_min(1e-8)
        return hybrid, adj_var

    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        h = self.temporal(x)
        adj, adj_var = self._adjacency()
